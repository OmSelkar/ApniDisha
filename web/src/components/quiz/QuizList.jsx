...
import { Loader2, Clock, Users, BookOpen, ArrowRight, Play, Star, Lock, Trophy, Zap, Heart, Brain, Target, Award } from "lucide-react";
...
const JOURNEY_STAGES = [
  ...
  },
  {
    ...
    description: "Test your strengths in different areas like logic, creativity, and analysis.",
    icon: Brain,
  ...
  },
  {
    ...
    description: "Match your interests and aptitudes to exciting career options.",
    icon: Target,
  ...
  },
  {
    ...
    description: "Get personalized recommendations for Arts, Science, Commerce, or Vocational streams.",
    icon: Award,
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        >
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-8">
            Embark on an exciting adventure to discover your perfect academic path and unlock your dream career!
          </p>
              ...
              return (
                ...
                >
                  ...
                  <div className={`flex-1 ${isLeft ? 'md:text-right' : 'md:text-left'}`}>
                    <motion.div
                      ...
                      className={`relative bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl border-2 ${
                        stage.isCompleted ? 'border-green-300 shadow-green-100' :
                        stage.isUnlocked ? 'border-indigo-300 shadow-indigo-100' : 'border-gray-300'
                      } p-8 max-w-lg mx-auto ${!stage.isUnlocked ? 'opacity-60' : ''}`}
                    >
                      {/* Badge */}
                      {stage.isCompleted && (
                        <motion.div
                      ...
                      {/* Icon */}
                      <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-r ${stage.color} text-white mb-4 shadow-lg`}>
                        <IconComponent className="h-8 w-8" />
                      </div>

                      <h3 className="text-2xl font-bold text-gray-800 mb-2">{stage.title}</h3>
                      <p className="text-indigo-600 font-medium mb-3">{stage.subtitle}</p>
                      ...
                      {stage.isUnlocked ? (
                        <Link to={`/quiz/${stage.id}`}>
                          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                            ...
                            >
                              {stage.isCompleted ? 'Review' : 'Start'} <ArrowRight className="ml-2 h-4 w-4" />
                            </Button>
                          </motion.div>
                        </Link>
                      ) : (
                        <Button disabled className="w-full bg-gray-300 text-gray-500 rounded-xl py-3 font-semibold">
                  ...
                  <div className="hidden md:flex items-center justify-center w-12 h-12 bg-white rounded-full shadow-lg border-4 border-indigo-200">
                    <div className={`w-6 h-6 rounded-full bg-gradient-to-r ${stage.color}`} />
                  </div>
        ...
        >
          <h2 className="text-3xl font-bold text-gray-800 mb-8">🏆 Your Achievements</h2>
